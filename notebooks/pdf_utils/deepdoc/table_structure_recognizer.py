...
class TableStructureRecognizer(Recognizer):
    ...
    def __init__(self):
        super().__init__(
            self.labels, "tsr", snapshot_download(repo_id="InfiniFlow/deepdoc")
        )
    ...
    def is_caption(bx):
        patt = [r"[图表]+[ 0-9:：]{2,}"]
        if (
            any([re.match(p, bx["text"].strip()) for p in patt])
        ...
        if len(rows) >= 4:
            ...
            while j < len(tbl[0]):
                ...
                if e > 1:
                    ...
                    continue
                f = (
                    j > 0 and tbl[ii][j - 1] and tbl[ii][j - 1][0].get("text")
                ) or j == 0
                ff = (
                    j + 1 < len(tbl[ii])
                    and tbl[ii][j + 1]
                    and tbl[ii][j + 1][0].get("text")
                ) or j + 1 >= len(tbl[ii])
                if f and ff:
                    j += 1
                    continue
                ...
                if j > 0 and not f:
                    for i in range(len(tbl)):
                        if tbl[i][j - 1]:
                            left = min(
                                left,
                                np.min([bx["x0"] - a["x1"] for a in tbl[i][j - 1]]),
                            )
                if j + 1 < len(tbl[0]) and not ff:
                    for i in range(len(tbl)):
                        if tbl[i][j + 1]:
                            right = min(
                                right,
                                np.min([a["x0"] - bx["x1"] for a in tbl[i][j + 1]]),
                            )
        ...
        if len(cols) >= 4:
            ...
            while i < len(tbl):
                ...
                if e > 1:
                    ...
                    continue
                f = (
                    i > 0 and tbl[i - 1][jj] and tbl[i - 1][jj][0].get("text")
                ) or i == 0
                ff = (
                    i + 1 < len(tbl)
                    and tbl[i + 1][jj]
                    and tbl[i + 1][jj][0].get("text")
                ) or i + 1 >= len(tbl)
                if f and ff:
                    i += 1
                    continue
        ...
        for i in range(len(tbl)):
            ...
            for j, arr in enumerate(tbl[i]):
                ...
                if arr:
                    ...
                    txt = " ".join(
                        [c["text"] for c in Recognizer.sort_Y_firstly(arr, h)]
                    )
                txts.append(txt)
                sp = ""
            ...
            for j in range(i + 1, rowno):
                ...
                for k in range(clmno):
                    ...
                    if len(headers[j][k]) > len(headers[j - 1][k]):
                        headers[j][k] += (de if headers[j][k] else "") + headers[j - 1][
                            k
                        ]
                    else:
                        headers[j][k] = (
